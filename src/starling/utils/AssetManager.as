package starling.utils
{
    ...
    public class AssetManager extends EventDispatcher
    {
		 ...
		 * suffixSortNumeric 如果run_0 run_1 run_2 - run_11这种情况不设true就会变成 run_0 run_1 run_10 run_11 - run_9
		 * */
        public function getTextures(prefix:String="", result:Vector.<Texture>=null, suffixSortNumeric:Boolean=false):Vector.<Texture>
        {
            if (result == null) result = new <Texture>[];
            
            for each (var name:String in getTextureNames(prefix, sNames, suffixSortNumeric))
			{
				//trace(name)
        ...
        public function getTextureNames(prefix:String="", result:Vector.<String>=null ,suffixSortNumeric=false):Vector.<String>
        {
            ...
            if(suffixSortNumeric)
			{
				result.sort(sortFun(prefix));
			}else
			{
		...
		private function sortFun(prefix:String):Function
		{
			var fun:Function = function (n1:String,n2:String):Number
			{
				return int(n1.split(prefix)[1]) - int(n2.split(prefix)[1]);
			}
			return fun;
		}
